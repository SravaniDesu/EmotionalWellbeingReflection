...
namespace Reflection.Helper
{
    ...
    public class DBHelper : IDataBase
    {
        ...
        public async Task<int> GetPeerReflectionFeedbackAsync(string channelId)
        {
            ReflectionDataRepository reflectionDataRepository = new ReflectionDataRepository(_configuration, _telemetry);
            FeedbackDataRepository feedbackDataRepository = new FeedbackDataRepository(_configuration, _telemetry);
            var todaysreflections = await reflectionDataRepository.GetAllActiveReflectionByChannelId(channelId);
            int feedback = 0;
            int count = 0;

            foreach(var reflection in todaysreflections)
            {
                Dictionary<int, List<FeedbackDataEntity>> feedbackData = await feedbackDataRepository.GetReflectionFeedback(reflection.ReflectionID) ?? null;

                foreach(var feedbackEntry in feedbackData)
                {
                    feedback += feedbackEntry.Key;
                    count++;
                }
            }
